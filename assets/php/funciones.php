...
function getRegistrospendientes()
{
    ...
    $sql = "SELECT COUNT(id) AS conteo FROM basecliente WHERE idcampania = {$_SESSION['campania_id_selected']} AND nagente = {$_SESSION['idEmpleado']} AND fecha_reagenda < NOW() UNION ALL SELECT COUNT(id) AS conteo FROM basecliente WHERE idcampania = {$_SESSION['campania_id_selected']} AND nagente = {$_SESSION['idEmpleado']} AND ultimoestatus = 'nuevo'";
...
function registrarMarcacion($peticion)
{
    ...
    if ($statusMarcacion == 'Venta' || $statusMarcacion == 'sinvalidar') {
        ...
        # administrador total
        if(isset($peticion['administrador_total'])){
            $sqlcaptura = "INSERT INTO marcador_manual.campos_adicionales(idfolio_externo, nombre_campo, valor_campo, tipocampo)VALUES";
            $conteoaa   = count($peticion['administrador_total']);
            $iad        = 0;
            foreach ($peticion['administrador_total'] as $key => $value) {
                $sqlcaptura .= "('{$idregistro}', 'administrador_total','{$value}', 'venta')";
                $iad++;
                if ($iad != count($peticion['administrador_total'])) {
                    $sqlcaptura .= ",";
                }
            }
            # echo $sqlcaptura."<hr>";
...
function getNumeroValidacion($contra_txt)
{
    ...
    echo json_encode($fila_sql);
}
function getStatus()
{
    $sqlp  = "SELECT ultimoestatus, COUNT(*) AS conteo FROM basecliente WHERE nagente = '{$_SESSION['idEmpleado']}' and idcampania = '{$_SESSION['campania_id_selected']}' GROUP BY ultimoestatus";
...
function updatebaselotes($cuenta){
    ...
    when fecha_asignacion_toque_1 is null || date_format(fecha_asignacion_toque_1,'%Y-%m-%d')= curdate()  then 'fecha_asignacion_toque_1,idmarcacionlast_1'
    when fecha_asignacion_toque_2 is null || date_format(fecha_asignacion_toque_2,'%Y-%m-%d')= curdate()  then 'fecha_asignacion_toque_2,idmarcacionlast_2'
    when fecha_asignacion_toque_3 is null || date_format(fecha_asignacion_toque_3,'%Y-%m-%d')= curdate()  then 'fecha_asignacion_toque_3,idmarcacionlast_3'
