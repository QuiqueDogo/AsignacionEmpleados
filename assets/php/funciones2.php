...
function registrarMarcacion($peticion)
{
    ...
    $sqlmm = "UPDATE marcador_manual.basecliente SET marcado=marcado+1, ultimoestatus = '{$statusMarcacion}', nsuper = '{$supervisor}', FechaUltimoStatus = current_timestamp ,AgenteUltimoStatus ='{$_SESSION['idEmpleado']}' ";
    ...
    if ($statusMarcacion == 'Venta' || $statusMarcacion == 'sinvalidar') {
        ...
        // tv adicional
        if (isset($peticion['servicio_tv_adicional'])) {
        ...
        // gamefly
        if (isset($peticion['gamefly_plan'])) {
            if (strlen($peticion['gamefly_plan']) != 0) {
    ...
    if (isset($_REQUEST['input_total']) && isset($_REQUEST['input_diferencia'])) {
        $montoNuevo         = $_REQUEST['input_total'] + $_REQUEST['input_diferencia'];
        $fechaventaoriginal = date('Y-m-d');
        $sql                = "UPDATE basecliente SET diferencia = '{$_REQUEST['input_total']}', montonuevo = '{$montoNuevo}', fechaventaoriginal = '{$fechaventaoriginal}', nsuper = '{$supervisor}' WHERE id = '{$idregistro}'";
