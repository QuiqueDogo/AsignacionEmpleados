...
document.addEventListener('DOMContentLoaded', function(event){
	...
	contenedorMadre.addEventListener('keypress',function(eve){
		if(eve.key == 'Enter'){
			eve.preventDefault();
		}
	});
...
function manejador__select_onchange(elemento){
	...
	campanias_permitidas = ['176','22','206','211'];
	...
	switch(elemento.selectedIndex){
		case 1:
		if(campanias_permitidas.includes(campania_id_marcador.innerHTML)){
			...
			if(campania_id_marcador.innerHTML == "189" || campania_id_marcador.innerHTML == "190" || campania_id_marcador.innerHTML == "203"){
...
function calcularPrecio(){
	...
	selectboof  	=  	document.querySelector('#adicional__bo_oferta_realizada');
	selectboadc 	= 	document.querySelector('#adicional__bo_internet_crecelo');
	selectbogam		= 	document.querySelector('#adicional__bo_gamefly_plan');
	selectbotvp		=	document.querySelector('#adicional__bo_adicional_paqueteTelevision');

	//////////////////////////////////////////////////////copia seleccionado a validador
	if(selectboof !=  null){
		selectboof.selectedIndex=parseInt(document.querySelector('#oferta_realizada').selectedIndex);
	}
	//////////////////////////////////////////////////////copia seleccionado a validador
	if(selectboadc!=  null){
		selectboadc.selectedIndex=parseInt(document.querySelector('#internet_crecelo').selectedIndex);
	}
	////////////////////////////////////////////////////////copia seleccionado a validador
	if(selectbogam != null){
		selectbogam.selectedIndex=parseInt(document.querySelector('#gamefly_plan').selectedIndex);
	}
	//////////////////////////////////////////////////////copia seleccionado a validador
	if(selectbotvp != null){
		selectbotvp.selectedIndex=parseInt(document.querySelector('#adicional_paqueteTelevision').selectedIndex);	
	}
	...
	//////////////////////////hace copia a validador
	let misChecksPadres = document.querySelectorAll(".checkinn input[type=checkbox]");
	misChecksPadres.forEach(elementoCopia => {
	//se valida si el id del chek esta en la lista de la validación
	if(elemento.id.substring(0, 2) == elementoCopia.id.substring(0, 2)){
		elementoCopia.checked = elemento.checked;
	  //valida si tiene un select asociado
	  let elselectex = elemento.parentNode.querySelector('select');
	  if(elselectex != null){
	  	let select1El_copia = elementoCopia.parentNode.querySelector('select');
	  	let select1El = elemento.parentNode.querySelector('select');
	  	select1El_copia.style.display = 'block';
	  	select1El_copia.value = select1El.value;
	  }
	}
...
function manejador__btnsend_click(){
	select_opciones  = document.querySelector('#oferta_realizada');
	opcion_seleccionada = select_opciones[oferta_realizada.selectedIndex].value;

	if(opcion_seleccionada == ''){
		alert('Es Necesario seleccionar una oferta realizada');
	}else{
...
function manejador_validarcontrasenia(response){
	...
	if(campania_id == 22){
		ofertas_bo				= document.querySelector('#adicional__bo_oferta_realizada');
		oferta_finalbo = ofertas_bo[ofertas_bo.selectedIndex].value;
		nombre_ofertabo 		=	ofertas_bo[ofertas_bo.selectedIndex].innerHTML;

		SelectCrecelo = document.querySelector('#adicional__bo_internet_crecelo');
		internet_crecelobo = SelectCrecelo[SelectCrecelo.selectedIndex].value;

		SelectGame = document.querySelector('#adicional__bo_gamefly_plan');
		Gamefly = SelectGame[SelectGame.selectedIndex].value;

		SelectTv = document.querySelector('#adicional__bo_adicional_paqueteTelevision');
		PaqueteTv = SelectTv[SelectTv.selectedIndex].value;
		total_bo = document.querySelector('.precio_totaltxt_bo').value;

		checks = document.querySelectorAll('input[type=checkbox]');
		checksSelect =['23_bo','27_bo','34_bo','49_bo','50_bo'];

		final_checks = [];

		checks.forEach(check =>{
			if(check.checked){
				if(check.id.includes('bo')){
					arreglochecks = {};
					if (checksSelect.includes(check.id)) {
						select = document.querySelector('#'+check.className+'_0');
						arreglochecks.nombre_campo = check.className+'_0';
						arreglochecks.valor_campo = select[select.selectedIndex].value;
					}else{
						arreglochecks.nombre_campo = check.id;
						arreglochecks.valor_campo = check.dataset.valor;
					}
					final_checks.push(arreglochecks);
				}
			}else{
			}
		});
	...
	switch(campania_id){
		...
		input_paquete_bo 		=	document.createElement('input');
		input_nombrepaquete_bo 	=	document.createElement('input');	
		input_internet_bo		=	document.createElement('input');
		input_gamefly_bo		=	document.createElement('input');
		input_paquetetv_bo		=	document.createElement('input');
		input_total_bo			=	document.createElement('input');

		input_paquete_bo.setAttribute('type','hidden');
		input_nombrepaquete_bo.setAttribute('type','hidden');
		input_internet_bo.setAttribute('type','hidden');
		input_gamefly_bo.setAttribute('type','hidden');
		input_paquetetv_bo.setAttribute('type','hidden');
		input_total_bo.setAttribute('type','hidden');


		input_paquete_bo.value =	oferta_finalbo;
		input_nombrepaquete_bo.value =	nombre_ofertabo;
		input_internet_bo.value =	internet_crecelobo;
		input_gamefly_bo.value =	Gamefly;
		input_paquetetv_bo.value = PaqueteTv;
		input_total_bo.value =	total_bo;


		input_paquete_bo.setAttribute('name','input_paquete_bo');
		input_nombrepaquete_bo.setAttribute('name','input_nombrepaquete_bo');
		input_internet_bo.setAttribute('name','input_internet_bo');
		input_gamefly_bo.setAttribute('name','input_gamefly_bo');
		input_paquetetv_bo.setAttribute('name','input_paquetetv_bo');
		input_total_bo.setAttribute('name','input_total_bo');

		if(input_paquete_bo != null){
			input_paquete_bo.value = oferta_finalbo;
		}
		if(input_nombrepaquete_bo != null){
			input_nombrepaquete_bo.value = nombre_ofertabo;
		}
		if(input_internet_bo != null){
			input_internet_bo.value = internet_crecelobo;
		}
		if(input_gamefly_bo != null){
			input_gamefly_bo.value = Gamefly;
		}
		if(input_paquetetv_bo != null){
			input_paquetetv_bo.value = PaqueteTv;
		}
		if(input_total_bo != null){
			input_total_bo.value = total_bo;
		}

		formulario.appendChild(input_paquete_bo);
		formulario.appendChild(input_nombrepaquete_bo);
		formulario.appendChild(input_internet_bo);
		formulario.appendChild(input_gamefly_bo);
		formulario.appendChild(input_paquetetv_bo);
		formulario.appendChild(input_total_bo);
		...
		input_total.value = precio_totaltxt;
		input_diferencia.value = oferta_realizada;
		final_checks.forEach(function(element,index){
			input_gen = document.createElement('input');
			input_gen.setAttribute('name',element.nombre_campo);
			input_gen.setAttribute('type', 'hidden');
			input_gen.value = element.valor_campo;
			formulario.appendChild(input_gen);
		});
	...
	input_campania =document.createElement('input');
	input_campania.setAttribute('name','input_campania');
	input_campania.setAttribute('type','hidden');
	input_campania.value =	campania_id;
	formulario.appendChild(input_campania);
...
function agregar_inputs_bo(){
	...
	input_tvadicional.id ="adicional_tvadicional_bo";
	let input_internetcrecelo = document.createElement('input');
	input_internetcrecelo.setAttribute('name', 'adicional_internet_crecelo_bo');
	input_internetcrecelo.id= 'adicional_internet_crecelo_bo';
	let tv_adicional_bo = document.querySelector('#adicional_tvadicional_bo').value;
